...
export function ChatPreview({ endpoint, defaultTemplate }: ChatPreviewProps) {
	const fetcher = (url: string) => fetch(url).then((res) => res.text());
	const { data = '', error } = useSWR(
		() => (isValidUrl(endpoint) ? endpoint : null),
		fetcher,
	);
	const { chatText = '' } = useWatch<CommandGeneratorFormData>();
	const template = chatText || defaultTemplate;
